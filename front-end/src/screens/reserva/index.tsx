...
const Reserva = () => {
  ...
  const [horariosSem, setHorarioSem] = useState<IHorario[]>([
    { horario: "", select: false },
  ]);
  const [horariosFim, setHorariosFim] = useState<IHorario[]>([
    { horario: "", select: false },
  ]);
  ...
  //Consumo de api cadastrando uma nova reserva
    ...
    if (storedToken !== null) {
      ...
      try {
        ...
        const response = await axios(
          "http://192.168.1.153:8080/novaReserva",
          OptionsRegister
        );
  ...
  return (
    <View style={{ flex: 1, height: "100%" }}>
      ...
      >
            ...
            >
              ...
              >
                <Arrow
                  ...
                  size={25}
                  color="#fff"
                  style={{ zIndex: 10, marginBottom: 5, pointerEvents: "none" }}
                />
                {formulario?.data !== "" ? (
                  <Text
                    style={{
                ...
                {mesas.map((e: any) => {
                  return (
                    <Picker.Item
                      key={e?.id}
                      style={{ fontSize: 15, color: "#fff" }}
                      label={`Número: ${e?.numero}: Descrição: ${e?.descricao}`}
                      value={e?.id}
                    />
                  );
                })}
            ...
            >
              {horariosSem?.map((e: any) => {
                return (
                  ...
                  >
                    <TextStyled select={e.select}>{e.horario}</TextStyled>
                  </TouchableOpacityStyled>
                );
              })}
            </View>
        ...
        >
          <ButtonText style={styles.buttonText}>Reservar</ButtonText>
        </ButtonContainer>
      </ScrollView>
    </View>
  );
